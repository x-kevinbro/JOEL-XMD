...
const prefix = config.PREFIX; const antiDeleteGlobal = config.ANTI_DELETE;

const demonContext = { forwardingScore: 999, isForwarded: true, forwardedNewsletterMessageInfo: { newsletterJid: '120363317462952356@newsletter', newsletterName: "ᴊᴏᴇʟ xᴅ ʙᴏᴛ ᴠ ⁷", serverMessageId: 143 } };

class DemonAntiDelete { constructor() { this.enabled = false; this.messageCache = new Map(); this.cacheExpiry = 5 * 60 * 1000; // 5 minutes this.cleanupInterval = setInterval(() => this.cleanExpiredMessages(), this.cacheExpiry); }
...
const demonDelete = new DemonAntiDelete(); const statusPath = './demon_antidelete.json';

let statusData = {}; if (fs.existsSync(statusPath)) { statusData = JSON.parse(fs.readFileSync(statusPath)); } if (!statusData.chats) statusData.chats = {};
...
const getChatInfo = async (jid) => { if (!jid) return { name: 'Unknown Chat', isGroup: false }; if (jid.includes('@g.us')) { try { const groupMetadata = await Matrix.groupMetadata(jid); return { name: groupMetadata?.subject || 'Demon Slayer Corps', isGroup: true }; } catch { return { name: 'Demon Slayer Corps', isGroup: true }; } } return { name: 'Private Mission', isGroup: false }; };

if (m.body.toLowerCase() === ${prefix}antidelete on || m.body.toLowerCase() === ${prefix}antidelete off) { const responses = { on: { text: 🗡️ *Joel xmd Anti-Delete Activated*\n\n + • Protection: *Enabled*\n + • Cache: *5 minutes*\n + • Scope: *All Chats*\n\n + _Deleted messages will now be recovered_\n\n + ⚔️ *Demon Slayer Corps*, contextInfo: demonContext }, off: { text: 🌑 *Joel xmd Anti-Delete Deactivated*\n\n + _Message recovery is now disabled_\n\n + ⚔️ *Demon Slayer Corps*, contextInfo: demonContext } };
...
Matrix.ev.on('messages.upsert', async ({ messages }) => { if (!antiDeleteGlobal && !demonDelete.enabled) return; if (!messages?.length) return;
...
Matrix.ev.on('messages.update', async (updates) => { if (!antiDeleteGlobal && !demonDelete.enabled) return; if (!updates?.length) return;
...
for (const update of updates) {
  try {
    ...
    const baseInfo =
      `╭━━━[ *ᴅᴇʟᴇᴛᴇᴅ ᴍᴇssᴀɢᴇ ʀᴇᴠɪᴠᴇᴅ* ]━━━╮\n` +
      `┃ 🧾 *ꜱᴇɴᴛ ʙʏ:* ${cachedMsg.senderFormatted}\n` +
      `┃ 🗑️ *ᴅᴇʟᴇᴛᴇᴅ ʙʏ:* ${deletedBy}\n` +
      `┃ 🏠 *ᴄʜᴀᴛ:* ${chatInfo.name}${chatInfo.isGroup ? ' (ɢʀᴏᴜᴘ)' : ''}\n` +
      `┃ ⏱️ *ꜱᴇɴᴛ:* ${demonDelete.formatTime(cachedMsg.timestamp)}\n` +
      `┃ ❌ *ᴅᴇʟᴇᴛᴇᴅ:* ${demonDelete.formatTime(Date.now())}\n` +
      `╰━━━━━━━━━━━━━━━━━━━━╯\n\n` +
      `${cachedMsg.content ? '💬 *ᴍᴇssᴀɢᴇ:*' : ''}\n` +
      `${cachedMsg.content || ''}\n\n` +
...
//update by joel james tech crds to demon slayer 
export default AntiDelete;
